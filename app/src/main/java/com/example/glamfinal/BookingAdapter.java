...
public class BookingAdapter extends RecyclerView.Adapter<BookingAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.tvServices.setText("Services : " + booking.getServices());
        holder.tvDate.setText("Date : " + booking.getDate());
        holder.tvTime.setText("Time of Appointment : " + booking.getTime());
        holder.tvTotalPrice.setText(String.format(Locale.US, "Total: Rs%.2f", booking.getTotalPrice()));
        holder.tvReceiptUrl.setText("View Receipt");

        if(booking.getReceiptUrl() != null) {
            holder.tvReceiptUrl.setOnClickListener(v -> {
                if(booking.getReceiptUrl() == null) {
                    Toast.makeText(context, "Cash will be payed after service", Toast.LENGTH_SHORT).show();
                }
                else {
    ...
    private void updateBookingStatus(Booking booking, String status) {
        if (booking == null || booking.getPhoneNumber() == null) {
            // Handle null booking object or phoneNumber not set
            return;
        }
